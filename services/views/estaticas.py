...
from ..serializers.calculo import CalculoSerializer
...
class ExemploSaudacao(APIView):
    def get(self, request):
        return Response({'saudacao': 'Ola mundo com classe'})


@api_view(['GET'])
def api_root(request, format=None):
    return Response({
        'saudacao': reverse('services:saudacao',
                            request=request, format=format),
        'saudacao_classe': reverse('services:saudacao_classe',
                                   request=request, format=format),
    })
