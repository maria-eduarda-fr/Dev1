...
def contact(request):
    if request.method == 'POST':
        ...
        if forms.is_valid():
            subject = forms.cleaned_data.get('subject') #validação - garantir que o usuario não inseriu dados não permitidos
        ...
        context = {'form': forms,
                   'url_form': 'contato' #faz referencia ao name usado na url em especifico(para ter template reutilizaveis)
        }
